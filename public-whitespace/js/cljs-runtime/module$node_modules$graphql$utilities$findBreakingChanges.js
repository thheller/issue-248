shadow$provide["module$node_modules$graphql$utilities$findBreakingChanges"] = function(global, process, require, module, exports) {
  ...
  DIRECTIVE_REMOVED:"DIRECTIVE_REMOVED", DIRECTIVE_ARG_REMOVED:"DIRECTIVE_ARG_REMOVED", DIRECTIVE_LOCATION_REMOVED:"DIRECTIVE_LOCATION_REMOVED", NON_NULL_DIRECTIVE_ARG_ADDED:"NON_NULL_DIRECTIVE_ARG_ADDED"};
  var DangerousChangeType = exports.DangerousChangeType = {ARG_DEFAULT_VALUE_CHANGE:"ARG_DEFAULT_VALUE_CHANGE", VALUE_ADDED_TO_ENUM:"VALUE_ADDED_TO_ENUM", INTERFACE_ADDED_TO_OBJECT:"INTERFACE_ADDED_TO_OBJECT", TYPE_ADDED_TO_UNION:"TYPE_ADDED_TO_UNION", NULLABLE_INPUT_FIELD_ADDED:"NULLABLE_INPUT_FIELD_ADDED", NULLABLE_ARG_ADDED:"NULLABLE_ARG_ADDED"};
  function findBreakingChanges(oldSchema, newSchema) {
    return [].concat(findRemovedTypes(oldSchema, newSchema), findTypesThatChangedKind(oldSchema, newSchema), findFieldsThatChangedTypeOnObjectOrInterfaceTypes(oldSchema, newSchema), findFieldsThatChangedTypeOnInputObjectTypes(oldSchema, newSchema).breakingChanges, findTypesRemovedFromUnions(oldSchema, newSchema), findValuesRemovedFromEnums(oldSchema, newSchema), findArgChanges(oldSchema, newSchema).breakingChanges, findInterfacesRemovedFromObjectTypes(oldSchema, newSchema), findRemovedDirectives(oldSchema, 
    newSchema), findRemovedDirectiveArgs(oldSchema, newSchema), findAddedNonNullDirectiveArgs(oldSchema, newSchema), findRemovedDirectiveLocations(oldSchema, newSchema));
  }
  ...
  function findRemovedTypes(oldSchema, newSchema) {
    ...
    Object.keys(oldTypeMap).forEach(function(typeName) {
      if (!newTypeMap[typeName]) {
        breakingChanges.push({type:BreakingChangeType.TYPE_REMOVED, description:typeName + " was removed."});
      }
    });
  ...
  function findTypesThatChangedKind(oldSchema, newSchema) {
    ...
    Object.keys(oldTypeMap).forEach(function(typeName) {
      ...
      if (oldType.constructor !== newType.constructor) {
        breakingChanges.push({type:BreakingChangeType.TYPE_CHANGED_KIND, description:typeName + " changed from " + (typeKindName(oldType) + " to " + typeKindName(newType) + ".")});
      }
    });
    return breakingChanges;
  }
    ...
    Object.keys(oldTypeMap).forEach(function(typeName) {
      ...
      Object.keys(oldTypeFields).forEach(function(fieldName) {
        ...
        oldTypeFields[fieldName].args.forEach(function(oldArgDef) {
          ...
          if (!newArgDef) {
            breakingChanges.push({type:BreakingChangeType.ARG_REMOVED, description:oldType.name + "." + fieldName + " arg " + (oldArgDef.name + " was removed")});
          } else {
            var isSafe = isChangeSafeForInputObjectFieldOrFieldArg(oldArgDef.type, newArgDef.type);
            if (!isSafe) {
              breakingChanges.push({type:BreakingChangeType.ARG_CHANGED_KIND, description:oldType.name + "." + fieldName + " arg " + (oldArgDef.name + " has changed type from ") + (oldArgDef.type.toString() + " to " + newArgDef.type.toString())});
            } else {
              if (oldArgDef.defaultValue !== undefined && oldArgDef.defaultValue !== newArgDef.defaultValue) {
                dangerousChanges.push({type:DangerousChangeType.ARG_DEFAULT_VALUE_CHANGE, description:oldType.name + "." + fieldName + " arg " + (oldArgDef.name + " has changed defaultValue")});
              }
            }
          }
        });
        newTypeFields[fieldName].args.forEach(function(newArgDef) {
          ...
          if (!oldArgDef) {
            if ((0, _definition.isNonNullType)(newArgDef.type)) {
              breakingChanges.push({type:BreakingChangeType.NON_NULL_ARG_ADDED, description:"A non-null arg " + newArgDef.name + " on " + (newType.name + "." + fieldName + " was added")});
            } else {
              dangerousChanges.push({type:DangerousChangeType.NULLABLE_ARG_ADDED, description:"A nullable arg " + newArgDef.name + " on " + (newType.name + "." + fieldName + " was added")});
            }
          }
        });
      });
    ...
    Object.keys(oldTypeMap).forEach(function(typeName) {
      ...
      Object.keys(oldTypeFieldsDef).forEach(function(fieldName) {
        if (!(fieldName in newTypeFieldsDef)) {
          breakingChanges.push({type:BreakingChangeType.FIELD_REMOVED, description:typeName + "." + fieldName + " was removed."});
        } else {
          ...
          if (!isSafe) {
            ...
            breakingChanges.push({type:BreakingChangeType.FIELD_CHANGED_KIND, description:typeName + "." + fieldName + " changed type from " + (oldFieldTypeString + " to " + newFieldTypeString + ".")});
          }
        }
      });
    });
      ...
      Object.keys(newTypeFieldsDef).forEach(function(fieldName) {
        if (!(fieldName in oldTypeFieldsDef)) {
          if ((0, _definition.isNonNullType)(newTypeFieldsDef[fieldName].type)) {
            breakingChanges.push({type:BreakingChangeType.NON_NULL_INPUT_FIELD_ADDED, description:"A non-null field " + fieldName + " on " + ("input type " + newType.name + " was added.")});
          } else {
            dangerousChanges.push({type:DangerousChangeType.NULLABLE_INPUT_FIELD_ADDED, description:"A nullable field " + fieldName + " on " + ("input type " + newType.name + " was added.")});
          }
        }
      });
    });
    ...
    Object.keys(oldTypeMap).forEach(function(typeName) {
      ...
      oldType.getTypes().forEach(function(type) {
        if (!typeNamesInNewUnion[type.name]) {
          typesRemovedFromUnion.push({type:BreakingChangeType.TYPE_REMOVED_FROM_UNION, description:type.name + " was removed from union type " + typeName + "."});
        }
      });
    });
    ...
    Object.keys(newTypeMap).forEach(function(typeName) {
      ...
      newType.getTypes().forEach(function(type) {
        if (!typeNamesInOldUnion[type.name]) {
          typesAddedToUnion.push({type:DangerousChangeType.TYPE_ADDED_TO_UNION, description:type.name + " was added to union type " + typeName + "."});
        }
      });
    });
    ...
    Object.keys(oldTypeMap).forEach(function(typeName) {
      ...
      oldType.getValues().forEach(function(value) {
        if (!valuesInNewEnum[value.name]) {
          valuesRemovedFromEnums.push({type:BreakingChangeType.VALUE_REMOVED_FROM_ENUM, description:value.name + " was removed from enum type " + typeName + "."});
        }
      });
    });
    ...
    Object.keys(oldTypeMap).forEach(function(typeName) {
      ...
      newType.getValues().forEach(function(value) {
        if (!valuesInOldEnum[value.name]) {
          valuesAddedToEnums.push({type:DangerousChangeType.VALUE_ADDED_TO_ENUM, description:value.name + " was added to enum type " + typeName + "."});
        }
      });
    });
    ...
    Object.keys(oldTypeMap).forEach(function(typeName) {
      ...
      oldInterfaces.forEach(function(oldInterface) {
        if (!newInterfaces.some(function(int) {
          ...
          breakingChanges.push({type:BreakingChangeType.INTERFACE_REMOVED_FROM_OBJECT, description:typeName + " no longer implements interface " + (oldInterface.name + ".")});
        }
      });
    });
    return breakingChanges;
    ...
    Object.keys(newTypeMap).forEach(function(typeName) {
      ...
      newInterfaces.forEach(function(newInterface) {
        if (!oldInterfaces.some(function(int) {
          ...
          interfacesAddedToObjectTypes.push({type:DangerousChangeType.INTERFACE_ADDED_TO_OBJECT, description:newInterface.name + " added to interfaces implemented " + ("by " + typeName + ".")});
        }
      });
    });
  ...
  function findRemovedDirectives(oldSchema, newSchema) {
    ...
    oldSchema.getDirectives().forEach(function(directive) {
      if (!newSchemaDirectiveMap[directive.name]) {
        removedDirectives.push({type:BreakingChangeType.DIRECTIVE_REMOVED, description:directive.name + " was removed"});
      }
    });
  ...
  function findRemovedDirectiveArgs(oldSchema, newSchema) {
    ...
    newSchema.getDirectives().forEach(function(newDirective) {
      ...
      findRemovedArgsForDirective(oldDirective, newDirective).forEach(function(arg) {
        removedDirectiveArgs.push({type:BreakingChangeType.DIRECTIVE_ARG_REMOVED, description:arg.name + " was removed from " + newDirective.name});
      });
    });
  ...
  function findAddedNonNullDirectiveArgs(oldSchema, newSchema) {
    ...
    newSchema.getDirectives().forEach(function(newDirective) {
      ...
      findAddedArgsForDirective(oldDirective, newDirective).forEach(function(arg) {
        ...
        addedNonNullableArgs.push({type:BreakingChangeType.NON_NULL_DIRECTIVE_ARG_ADDED, description:"A non-null arg " + arg.name + " on directive " + (newDirective.name + " was added")});
      });
    });
  ...
  function findRemovedDirectiveLocations(oldSchema, newSchema) {
    ...
    newSchema.getDirectives().forEach(function(newDirective) {
      ...
      findRemovedLocationsForDirective(oldDirective, newDirective).forEach(function(location) {
        removedLocations.push({type:BreakingChangeType.DIRECTIVE_LOCATION_REMOVED, description:location + " was removed from " + newDirective.name});
      });
    });
    return removedLocations;
  }
